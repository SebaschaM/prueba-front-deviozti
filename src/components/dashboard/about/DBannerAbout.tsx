...
import { DataAboutPageI, HeroAbout } from "../../../interfaces/dataAbout";
...
const DBannerAbout = ({ dataPage, toast }: Props) => {
  const { handleUpdateHero } = useAbout();
  const [dataBanner, setDataBanner] = useState<HeroAbout>();
  const { register, handleSubmit, setValue } = useForm<HeroAbout>();
  ...
  const onSubmit = async (data: HeroAbout | any) => {
